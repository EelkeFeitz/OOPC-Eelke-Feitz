...
TEST_CASE( "add empty vector into vector" ) {
    vector v( 5, 5);
    vector x = ( v + vector( 0, 0 ) );
    REQUIRE( x == vector( 5, 5 ) );
}

TEST_CASE( "add a vector into vector; checks if origional vector stays the same" ) {
    vector v( 5, 5 );
    vector x = ( v + vector( 2, 2 ) );
    REQUIRE( v == vector( 5, 5 ) );
}   

TEST_CASE( "add vector into vector; checks if vector gets adjusted") {
    vector v( 5, 5 );
    vector x( 2, 2 );
    x += v;
...
TEST_CASE( "multiplies a vector with an int, returns a vector" ) {
    vector v( 5, 5 );
    vector x = ( v * 2 );
    REQUIRE( x == vector( 10, 10 ) );
}

TEST_CASE( "multiplies a int with a vector, returns a vector" ) {
    int x = 2;
    vector v = ( x * vector( 5, 5 ) );
    REQUIRE( v == vector( 10, 10) );
}

TEST_CASE( "adds nothing to a vector" ) {
    vector v( 5, 5 );
    vector x = + v ;
    REQUIRE( x == vector( 5, 5) );
}